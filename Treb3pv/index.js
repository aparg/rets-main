...
const getPropertyFieldsMetadata = async (propertyType) => {
  createConsoleLog(
    __filename,
    `Retrieving available fields for property type: ${propertyType}`
  );
  await isLoggedIn(
    USERNAME_FOR_DATA_RETRIEVAL,
    PASSWORD_FOR_DATA_RETRIEVAL,
  ...
  try {
    ...
    createConsoleLog(
      __filename,
      `Retrieved available fields for property type: ${propertyType}`
    );

    return {
      message: "Successfully retrieved residential metadata",
      data: response.data,
    };
  } catch (error) {
...
const getPropertyData = async (propertyType, timeStampSql) => {
  createConsoleLog(
    __filename,
    `getting property data for ${propertyType}, updated after ${timeStampSql}`
  );
  await isLoggedIn(
    USERNAME_FOR_DATA_RETRIEVAL,
    PASSWORD_FOR_DATA_RETRIEVAL,
  ...
  try {
    ...
    if (replyCodeIndex !== -1) {
      ...
      const replyCode = xmlResponse.substring(
        replyCodeStartIndex,
        replyCodeEndIndex
      );
      if (replyCode === "20201") {
...
const getImages = async (propertyMlsArray, photoDirectoriesName) => {
  ...
  const formattedPropertyMls = propertyMlsArray
    .map((mls) => `${mls}:*`)
    .join(",");
  ...
  try {
    ...
    if (boundaryMatch && boundaryMatch[1]) {
      ...
      createConsoleLog(
        __filename,
        `received ${parts.length} number of objects in response.`
      );
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (imageTypeMatch) {
          ...
          const filename = path.join(
            __dirname,
            `../Data/${photoDirectoriesName}`,
            `Photos/${contentId}-${objectId}.${imageFormat}`
          );
          ...
          if (updatedImagesObject.hasOwnProperty(contentId)) {
            ...
            updatedImagesObject[contentId].push(
              `${contentId}-${objectId}.${imageFormat}`
            );
          } else {
            // If it doesn't exist, create a new key-value pair with an array containing the image filename
            updatedImagesObject[contentId] = [
              `${contentId}-${objectId}.${imageFormat}`,
            ];
          }
        }
...
const getAllActiveListings = async (propertyType) => {
  createConsoleLog(
    __filename,
    `retrieving inactive listings for ${propertyType}`
  );
  await isLoggedIn(
    USERNAME_FOR_INACTIVE_RETRIEVAL,
    PASSWORD_FOR_INACTIVE_RETRIEVAL,
  ...
  try {
    ...
    createConsoleLog(
      __filename,
      `retrieved inactive listings for ${propertyType}`
    );
